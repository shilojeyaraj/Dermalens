...
def get_supabase_auth() -> Client:
    ...
    if supabase_auth is None:
        try:
            ...
            print(
                f"[AUTH] URL parsed - scheme: {urlparse(SUPABASE_URL).scheme}, netloc: {urlparse(SUPABASE_URL).netloc}"
            )
    ...
    if supabase_admin is None:
        try:
            ...
            supabase_admin = create_client(
                SUPABASE_URL, SUPABASE_SERVICE_KEY, options=supabase_options
            )
...
class AuthManager:
    ...
    async def sign_up(self, email: str, password: str) -> Dict:
        ...
        try:
            ...
            try:
                ...
                traceback.print_exc()
                return {"success": False, "error": f"Database connection failed: {str(conn_error)}"}
                ...
                try:
                    ...
                    if "Name or service not known" in error_str or "Errno -2" in error_str:
                        if attempt < max_retries - 1:
                            ...
                            print(
                                f" [AUTH] DNS error detected, waiting {wait_time}s before retry..."
                            )
                            time.sleep(wait_time)
                        else:
                            # Last attempt failed
                            raise Exception(
                                f"Failed to connect to Supabase after {max_retries} attempts. DNS resolution error: {error_str}"
                            )
                    else:
    ...
    async def sign_in(self, email: str, password: str) -> Dict:
        ...
        try:
            ...
            supabase = self._get_supabase()
            result = supabase.rpc(
                "authenticate_user_with_rls", {"user_email": email, "user_password": password}
            ).execute()
            ...
            if result.data and len(result.data) > 0:
                user_data = result.data[0]
                print(
                    f" [AUTH] User authenticated successfully with ID: {user_data.get('user_id')}"
                )
    ...
    async def get_user(self, access_token: str) -> Dict:
        ...
        try:
            ...
            if not user_id:
                return {"success": False, "error": "Invalid token - no user_id"}

            # Get user profile from database
            supabase = self._get_supabase()
            result = supabase.rpc("get_user_profile_with_rls", {"user_uuid": user_id}).execute()
