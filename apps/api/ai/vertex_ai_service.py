...
class VertexAISkinAnalysisService:
    ...
    async def analyze_skin_comprehensive(
        self, image_data: bytes, user_profile: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_basic_recommendations(
        self, conditions: List[str], user_profile: Optional[Dict[str, Any]]
    ) -> List[str]:
    ...
    async def _single_model_analysis(
        ...
        try:
            ...
            response = self.prediction_client.predict(
                endpoint=self.endpoints["skin_analysis"], instances=instances
            )
    ...
    async def _analyze_with_model(
        self, model_name: str, request_data: Dict[str, Any], analysis_type: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Make prediction
            response = self.prediction_client.predict(endpoint=endpoint, instances=[request_data])
    ...
    def _combine_ensemble_results(
        ...
        for model_name, result in ensemble_results.items():
            ...
            if "skin_type" in result:
                skin_types.append({"type": result["skin_type"], "weight": weight})
        ...
        for condition in all_conditions:
            ...
            if condition["severity"] == "severe":
                ...
                condition["severity"] == "moderate" and condition_votes[key]["severity"] != "severe"
            ):
        ...
        final_skin_type = (
            max(skin_type_votes.items(), key=lambda x: x[1])[0] if skin_type_votes else "normal"
        )
        ...
        combined["analysis_results"] = [
            {
                ...
                "skin_type": {
                    "primary": final_skin_type,
                    "confidence": max(skin_type_votes.values()) if skin_type_votes else 0.5,
                },
    ...
    async def _fallback_analysis(
        ...
        try:
            ...
            return gemini_service.analyze_skin_image(
                image_data=image_data, user_profile=user_profile, user_skin_profile=user_profile
            )
    ...
    async def _store_in_cache(self, cache_key: str, result: Dict[str, Any], ttl: int = 3600):
        ...
        try:
            ...
            await self.cache_client.setex(cache_key, ttl, json.dumps(result, default=str))
    ...
    def _generate_analysis_id(
        self, image_data: bytes, user_profile: Optional[Dict[str, Any]]
    ) -> str:
    ...
    async def _track_performance(
        self, analysis_id: str, start_time: float, end_time: float, result: Dict[str, Any]
    ):
    ...
    async def get_ai_recommendations(
        ...
        try:
            ...
            response = self.prediction_client.predict(
                endpoint=self.endpoints["recommendation_engine"], instances=instances
            )
