...
# Configuration
from config import ENSEMBLE_ENABLED, VERTEX_AI_ENABLED
...
class EnhancedProductRecommendationService:
    ...
    async def _analyze_user_needs(
        self, skin_analysis: Dict[str, Any], user_profile: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            primary_concerns = self._determine_primary_concerns(
                detected_conditions, skin_health_score
            )
            ...
            return {"success": False, "error": f"Needs analysis failed: {str(e)}"}
    ...
    def _determine_budget_preference(
        self, user_profile: Optional[Dict[str, Any]], health_score: float
    ) -> str:
    ...
    async def _search_products(
        ...
        try:
            ...
            if budget_range:
                ...
                filtered_products = [
                    p for p in all_products if min_budget <= p.get("price", 0) <= max_budget
                ]
            ...
            return {"success": False, "error": f"Product search failed: {str(e)}"}
    ...
    async def _search_elasticsearch(self, needs: Dict[str, Any], limit: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Elasticsearch search failed: {str(e)}"}
    ...
    def _calculate_personalization_score(
        self, product: Dict[str, Any], needs: Dict[str, Any], user_profile: Optional[Dict[str, Any]]
    ) -> float:
    ...
    def _generate_recommendation_reason(
        self, product: Dict[str, Any], needs: Dict[str, Any], score: float
    ) -> str:
    ...
    async def _generate_comprehensive_recommendations(
        self, products: List[ProductRecommendation], needs: Dict[str, Any], recommendation_type: str
    ) -> List[ProductRecommendation]:
    ...
    def _generate_usage_instructions(
        self, product: ProductRecommendation, needs: Dict[str, Any]
    ) -> str:
        ...
        if category == "cleanser":
            return (
                "Use morning and evening. Apply to wet skin, massage gently, then rinse thoroughly."
            )
    ...
    async def _cache_recommendations(
        self, recommendations: List[ProductRecommendation], needs: Dict[str, Any]
    ) -> None:
