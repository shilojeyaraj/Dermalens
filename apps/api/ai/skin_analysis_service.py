...
class SkinAnalysisService:
            ...
            if self.model is not None:
                ...
                for i in range(len(top_indices[0])):
                    ...
                    if prob > 0.3:  # Threshold for relevance
                        condition = self.skin_conditions[idx] if idx < len(self.skin_conditions) else f"condition_{idx}"
                        conditions.append({
                            "condition": condition,
    ...
    def analyze_skin_image(self, image_data: bytes, user_profile: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if analysis_results:
                overall_health = sum(result["health_score"] for result in analysis_results) / len(analysis_results)
            else:
                overall_health = 50
