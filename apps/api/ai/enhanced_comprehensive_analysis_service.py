...
# try/except block below (previously defined further down -> NameError on failure).
...
class EnhancedComprehensiveSkinAnalysisService:
    ...
    async def analyze_user_comprehensive(
        self, user_id: str, image_id: Optional[str] = None, analysis_type: str = "comprehensive"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            ai_analysis = await self._perform_enhanced_analysis(
                image_data["data"], user_data["skin_profile"], analysis_type
            )
            ...
            recommendations = await self._get_ai_recommendations(
                ai_analysis["analysis"], user_data["skin_profile"]
            )

            # Step 5: Generate enhanced routine
            routine = await self._generate_enhanced_routine(
                ai_analysis["analysis"], recommendations, user_data["skin_profile"]
            )
    ...
    async def _fetch_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to fetch user data: {str(e)}"}
    ...
    async def _get_image_data(self, user_id: str, image_id: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            image_data = await self._download_image_from_supabase(
                image_to_analyze["bucket"], image_to_analyze["storage_path"]
            )
            ...
            return {"success": False, "error": f"Failed to get image data: {str(e)}"}
    ...
    async def _perform_enhanced_analysis(
        self, image_data: bytes, user_skin_profile: Optional[Dict[str, Any]], analysis_type: str
    ) -> Dict[str, Any]:
    ...
    async def _fallback_analysis(
        self, image_data: bytes, user_skin_profile: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"All analysis methods failed: {str(e)}"}

    async def _get_ai_recommendations(
        self, analysis_result: Dict[str, Any], user_skin_profile: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            if self.vertex_ai_enabled:
                ...
                ai_recommendations = await self.vertex_ai.get_ai_recommendations(
                    analysis_result, user_skin_profile or {}
                )
            ...
            elasticsearch_result = self.elasticsearch.get_recommendations(
                user_profile=user_skin_profile or {}, analysis_results=[analysis_result], limit=10
            )

            # Get recommendations from Google Search
            google_products = await self._search_google_products(
                analysis_result.get("detected_conditions", []), user_skin_profile
            )
    ...
    async def _search_google_products(
        self, conditions: List[str], user_skin_profile: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for condition in conditions[:3]:  # Top 3 conditions
                result = self.search.search_products_for_conditions(
                    conditions=[condition], user_profile=user_skin_profile
                )
    ...
    async def _generate_enhanced_routine(
        ...
        try:
            ...
            moisturizers = [
                ...
                if "moisturizer" in p.get("name", "").lower()
                or "cream" in p.get("name", "").lower()
            ]
            sunscreens = [
                p
                for p in product_list
                if "sunscreen" in p.get("name", "").lower() or "spf" in p.get("name", "").lower()
            ]
