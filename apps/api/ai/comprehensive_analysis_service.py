...
class ComprehensiveSkinAnalysisService:
    ...
    def __init__(self):
        self.db = db_manager
        # Use Gemini if enabled, otherwise fallback to OpenAI
    ...
    async def analyze_user_by_id(
        self, user_id: str, image_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            user_skin_profile = (
                skin_profile_result["data"] if skin_profile_result["success"] else None
            )

            if user_skin_profile:
                logger.info(
                    f"Fetched skin profile with {len(user_skin_profile.get('allergies', []))} allergies"
                )
            ...
            logger.info(
                f"AI detected {len(analysis_result['analysis']['conditions_detected'])} skin conditions"
            )

            # Step 6: Generate personalized recommendations
            recommendations = self.ai.generate_personalized_recommendations(
                analysis_result, user_skin_profile
            )
            ...
            products = await self._search_products_for_recommendations(
                recommendations, user_skin_profile
            )

            # Step 8: Generate actionable routine
            routine = self._generate_skincare_routine(
                analysis_result["analysis"], products, user_skin_profile
            )
            ...
            return {
                ...
                "ai_analysis": {
                    ...
                    "recommended_ingredients": analysis_result["analysis"][
                        "recommended_ingredients"
                    ],
                    "recommended_products": analysis_result["analysis"][
    ...
    async def _search_products_for_recommendations(
        self, recommendations: Dict[str, Any], user_skin_profile: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if not self.search.is_enabled():
            ...
            return {"products": [], "search_queries_used": [], "source": "mock"}
        ...
        if conditions:
            result = self.search.search_products_for_conditions(
                conditions=conditions[:3], user_profile=user_skin_profile  # Top 3 conditions
            )
        ...
        unique_products.sort(
            key=lambda x: x.get("final_score", x.get("relevance_score", 0)), reverse=True
        )
    ...
    def _generate_skincare_routine(
        ...
        moisturizers = [
            ...
            if "moisturizer" in p.get("name", "").lower() or "cream" in p.get("name", "").lower()
        ]
        sunscreens = [
            p
            for p in product_list
