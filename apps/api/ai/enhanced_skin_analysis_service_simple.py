...
class EnhancedSkinAnalysisService:
    ...
    async def analyze_skin_image(
        ...
        try:
            ...
            ai_results = await self._perform_ai_analysis_simple(
                processed_image, user_profile, analysis_type
            )
            ...
            logger.info(
                f"   - Conditions detected: {len(analysis_data.get('detected_conditions', []))}"
            )
            logger.info(
                f"   - Confidence scores: {[c.get('confidence', 0) for c in analysis_data.get('conditions', [])]}"
            )

            # Step 3: Generate comprehensive report
            logger.info("📊 Step 3: Generating comprehensive report")
            report = await self._generate_comprehensive_report(
                analysis_data, user_profile, processed_image
            )
    ...
    async def _validate_and_preprocess_image(self, image_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            if quality_score < self.image_quality_threshold:
                logger.warning(
                    f"⚠️ Low image quality: {quality_score:.2f} < {self.image_quality_threshold}"
                )
    ...
    async def _calculate_image_quality_simple(self, image: Image.Image) -> Dict[str, Any]:
        ...
        try:
            ...
            brightness_score = min(
                max((brightness - self.min_brightness) / (255 - self.min_brightness), 0.0), 1.0
            )
            ...
            quality = (
                size_score * 0.3 + aspect_score * 0.2 + brightness_score * 0.3 + edge_norm * 0.2
            )
    ...
    async def _perform_ai_analysis_simple(
        ...
        try:
            ...
            if self.vertex_ai_enabled:
                ...
                return await self._vertex_ai_analysis_simple(
                    processed_image, user_profile, analysis_type
                )
            else:
                logger.info("   - Using fallback analysis")
                return await self._fallback_analysis_simple(
                    processed_image, user_profile, analysis_type
                )
    ...
    async def _generate_recommendations(
        self, analysis_data: Dict[str, Any], user_profile: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
