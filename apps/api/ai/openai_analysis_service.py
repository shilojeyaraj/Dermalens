...
from config import OPENAI_API_KEY, OPENAI_ENABLED, OPENAI_MODEL
...
class OpenAISkinAnalysisService:
    ...
    def analyze_skin_image(
        ...
        """
        if not self.enabled:
            return {"success": False, "error": "OpenAI API is not enabled", "analysis": None}
    ...
    def _build_analysis_prompt(
        self, user_profile: Optional[Dict[str, Any]], user_skin_profile: Optional[Dict[str, Any]]
    ) -> str:
        ...
        if user_skin_profile:
            prompt_parts.extend(["", "**USER PROFILE CONTEXT:**"])

            if user_skin_profile.get("skin_type"):
                prompt_parts.append(
                    f"- User reports skin type as: {user_skin_profile['skin_type']}"
                )
            ...
            if user_skin_profile.get("allergies"):
                ...
                prompt_parts.append(
                    f"- **IMPORTANT - User is allergic to: {allergies}** (DO NOT recommend products with these ingredients)"
                )
            ...
            if user_skin_profile.get("sensitivity_level"):
                prompt_parts.append(
                    f"- Sensitivity level: {user_skin_profile['sensitivity_level']}"
                )
    ...
    def _parse_analysis_response(self, analysis_text: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            "routine_suggestions": {"morning": [], "evening": []},
            "precautions": [],
    ...
    def generate_personalized_recommendations(
        self, analysis_result: Dict[str, Any], user_skin_profile: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not analysis_result.get("success"):
            return {"success": False, "error": "Analysis failed, cannot generate recommendations"}
