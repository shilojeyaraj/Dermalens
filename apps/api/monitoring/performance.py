...
try:
    ...
    logger.warning(
        "google-cloud-monitoring not installed. Google Cloud monitoring features will be disabled."
    )
    monitoring_v3 = None
    query = None
...
try:
    ...
    logger.warning(
        "prometheus-client not installed. Prometheus monitoring features will be disabled."
    )
    Counter = None
    Histogram = None
    Gauge = None
...
class PerformanceMonitoringService:
        ...
        try:
            if self.enabled and GOOGLE_MONITORING_AVAILABLE and monitoring_v3:
                ...
                if not GOOGLE_MONITORING_AVAILABLE:
                    logger.warning(
                        "[MONITORING] Google Cloud monitoring package not available - monitoring disabled"
                    )
                else:
    ...
    def _setup_prometheus_metrics(self):
        ...
        try:
            ...
            self.prometheus_metrics["cache_hit_rate"] = Gauge(
                "dermalens_cache_hit_rate", "Cache hit rate percentage"
            )
            ...
            self.prometheus_metrics["service_health"] = Gauge(
                "dermalens_service_health", "Service health status", ["service"]
            )
    ...
    async def track_analysis_performance(
        ...
        try:
            ...
            self.prometheus_metrics["analysis_requests"].labels(
                service=service, type=analysis_type, status="success" if success else "error"
            ).inc()

            self.prometheus_metrics["analysis_duration"].labels(
                service=service, type=analysis_type
            ).observe(duration)
            ...
            logger.info(
                f"📊 Tracked analysis performance: {service}/{analysis_type} - {duration:.2f}s"
            )
    ...
    async def track_cache_performance(
        self, operation: str, cache_key: str, hit: bool, duration: float, service: str
    ):
    ...
    async def track_recommendation_performance(
        ...
        try:
            ...
            self.prometheus_metrics["recommendation_requests"].labels(
                type=recommendation_type, status="success" if success else "error"
            ).inc()
    ...
    async def _send_metrics_to_cloud_monitoring(
        self, service: str, metrics: List[PerformanceMetric]
    ):
        """Send metrics to Google Cloud Monitoring"""
        try:
    ...
    async def get_performance_analytics(self) -> Dict[str, Any]:
        ...
        try:
            analytics = {
                ...
                "services_health": {
                    service: asdict(health) for service, health in self.health_status.items()
                },
        ...
        try:
            ...
            for service, stats in self.performance_history.items():
                if stats.get("avg_response_time", 0) > 5.0:
                    recommendations.append(
                        f"Consider optimizing {service} - avg response time: {stats['avg_response_time']:.2f}s"
                    )
    ...
    async def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "time_range": {
                    "earliest": (
                        min(m.timestamp for m in self.metrics_buffer).isoformat()
                        if self.metrics_buffer
                        else None
                    ),
                    "latest": (
                        max(m.timestamp for m in self.metrics_buffer).isoformat()
                        if self.metrics_buffer
                        else None
                    ),
                },
