...
try:
    ...
    try:
        ...
        print(
            "⚠️  Could not read .env with UTF-8/UTF-16; proceeding with existing environment variables."
        )
    ...
    try:
        ...
        for i in range(0, len(products), batch_size):
            ...
            try:
                ...
                print(
                    f"📤 Indexed batch {i//batch_size + 1}/{(len(products)-1)//batch_size + 1} ({len(batch)} products)"
                )
        ...
        test_result = elasticsearch_service.search_products(
            query="acne cleanser", skin_conditions=["acne"], skin_types=["oily"], size=5
        )
