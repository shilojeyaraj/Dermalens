...
async def analyze_skin(file: UploadFile = File(...)):
    ...
    try:
        if not SKIN_ANALYSIS_AVAILABLE:
            return JSONResponse(
                status_code=503,
                content={"error": "Skin analysis service not available"}
            )
...
async def recommend_products(
    ...
    try:
        if not PRODUCT_RECOMMENDATION_AVAILABLE:
            return JSONResponse(
                status_code=503,
                content={"error": "Product recommendation service not available"}
            )
...
async def get_profile(user_id: str):
    ...
    if not DATABASE_AVAILABLE:
        return JSONResponse(
            status_code=503,
            content={"error": "Database service not available"}
        )
