...
# Configure logging early so `logger` is available inside the import-time
# try/except blocks below (previously it was defined further down, which raised
# NameError if Elasticsearch import failed during startup).
...
async def health_check():
    ...
    try:
        ...
        healthy_services = [
            service for service, health in service_health.items() if health.status == "healthy"
        ]

        overall_status = (
            "healthy"
            if all(service in healthy_services for service in critical_services)
            else "degraded"
        )
    ...
    try:
        ...
        if analysis_type == "streaming" and VERTEX_AI_STREAMING_ENABLED:
            ...
            result = await enhanced_comprehensive_analysis_service.analyze_user_comprehensive(
                user_id=current_user_id, analysis_type=analysis_type
            )
...
async def analyze_skin_enhanced_detailed(
    ...
    try:
        ...
        user_profile = (
            user_profile_result.get("data") if user_profile_result.get("success") else None
        )
        ...
        analysis_result = await enhanced_skin_analysis_service.analyze_skin_image(
            image_data=content, user_profile=user_profile, analysis_type=analysis_type
        )
...
async def get_enhanced_recommendations(
    ...
    try:
        ...
        recommendations_result = (
            await enhanced_product_recommendation_service.get_enhanced_recommendations(
...
async def analyze_skin_multi_angle(
    ...
    try:
        ...
        for i, file in enumerate(files):
            ...
            analysis_result = await enhanced_skin_analysis_service.analyze_skin_image(
                image_data=content, user_profile=user_profile, analysis_type="comprehensive"
            )
            ...
            if analysis_result["success"]:
                analysis_results.append(
                    {
                        "angle": (
                            file.filename.split("_")[0] if "_" in file.filename else f"angle_{i}"
                        ),
        ...
        try:
            ...
            try:
                ...
                existing_profile = (
                    profile_result.get("data") if profile_result.get("success") else None
                )
                if existing_profile:
                    ...
                    await db_manager.update_skin_profile(
                        current_user_id, {"primary_concerns": merged}
                    )
                elif aggregated_conditions:
                    await db_manager.create_skin_profile(
                        current_user_id, {"primary_concerns": list(aggregated_conditions)}
                    )
                logger.info(
                    f"🧠 Updated skin profile with detected concerns: {list(aggregated_conditions)}"
                )
            except Exception as e:
        ...
        if user_profile:
            ...
            if user_profile.get("primary_concerns"):
                ...
                if isinstance(profile_concerns, list):
                    ...
                    combined_concerns.extend(
                        [c.strip() for c in profile_concerns.split(",") if c.strip()]
                    )
                else:
            ...
            logger.info(
                f"✅ Profile integration complete - Using: skin_type, primary_concerns, skin_concerns, allergies, routine_frequency"
            )
        ...
        if (
            recs.get("success")
            and recs.get("recommendations")
            and len(recs.get("recommendations", [])) > 0
        ):
            ...
            routine["total_products"] = len(routine["morning_routine"]) + len(
            ...
            skin_type = (
                user_profile.get("skin_type", "combination") if user_profile else "combination"
            )
            primary_concerns = (
                user_profile.get("primary_concerns", ["general_care"])
                if user_profile
                else ["general_care"]
            )
        ...
        if "total_products" not in routine:
            routine["total_products"] = len(routine.get("morning_routine", [])) + len(
                routine.get("evening_routine", [])
            )
        ...
        combined_result = {
            ...
            "analysis_notes": {
                ...
                "profile_enhancement": (
                    f"Enhanced with user profile data: {user_profile.get('skin_type', 'unknown')} skin type, {len(combined_concerns)} profile concerns"
                    if user_profile
        ...
        logger.info(
            f"✅ Multi-angle analysis completed: {len(analysis_results)}/{len(files)} successful"
        )
        logger.info(
            f"🎯 Generated {len(combined_result.get('recommendations', []))} recommendations"
        )
        logger.info(
            f"📋 Created {len(routine['morning_routine'])} morning + {len(routine['evening_routine'])} evening routine steps"
        )
...
async def generate_profile_recommendations(current_user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        routine = {"morning_routine": [], "evening_routine": []}
        rec_products = recs.get("recommendations", [])
            ...
            routine["morning_routine"].append(
                {
                    ...
                    "brand": sunscreen.get("brand"),
                    "url": sunscreen.get("url") or sunscreen.get("product_url"),
                    "instructions": "Apply SPF 30+ as last step.",
                }
            )
        if cleanser:
            routine["evening_routine"].append(
                {
                    ...
                    "brand": cleanser.get("brand"),
                    "url": cleanser.get("url") or cleanser.get("product_url"),
                    "instructions": "Cleanse to remove impurities.",
                }
            )
        if serum:
            routine["evening_routine"].append(
                {
                    ...
                    "brand": serum.get("brand"),
                    "url": serum.get("url") or serum.get("product_url"),
                    "instructions": "Apply treatment serum if tolerated.",
                }
            )
        if moisturizer:
            routine["evening_routine"].append(
    ...
    try:
        ...
        if brand_filters and result.get("success"):
            ...
            result["products"] = [
                ...
                if p.get("brand", "").lower() in requested_set
                or any(rb in p.get("brand", "").lower() for rb in requested_set)
            ]
        # Optional sorting
        ...
        def _map_product(p):
            ...
            skin_type = (
                ", ".join(skin_types_val)
                if isinstance(skin_types_val, list)
                else (p.get("skin_type") or "All Skin Types")
            )
...
async def analyze_comprehensive_enhanced(
    ...
    try:
        ...
        logger.info(
            f"   - Conditions detected: {len(skin_analysis.get('detected_conditions', []))}"
        )
...
async def _stream_analysis_results(content: bytes, user_id: str, analysis_id: str):
    ...
    try:
        ...
        async for result_chunk in vertex_ai_service._stream_predictions(
            [{"image": {"b64": content.decode()}, "user_id": user_id, "streaming": True}]
        ):
...
async def _generate_enhanced_routine(
    ...
    try:
        ...
        logger.error(f"❌ Enhanced routine generation failed: {e}")
        return {"morning": [], "evening": [], "error": f"Routine generation failed: {str(e)}"}
...
async def create_skin_profile(
    request: SkinProfileCreate, current_user: dict = Depends(get_current_user)
):
    """Create user skin profile"""
    result = await db_manager.create_skin_profile(
        current_user["id"], request.dict(exclude_unset=True)
    )
    if not result["success"]:
        raise HTTPException(
            status_code=400, detail=result.get("error", "Failed to create skin profile")
        )
    return result
...
async def update_skin_profile(
    request: SkinProfileUpdate, current_user: dict = Depends(get_current_user)
):
    ...
    if not existing["success"] or not existing["data"]:
        ...
        result = await db_manager.update_skin_profile(
            current_user["id"], request.dict(exclude_unset=True)
        )
        if not result["success"]:
            raise HTTPException(
                status_code=400, detail=result.get("error", "Failed to update skin profile")
            )
        return result
...
async def manual_seed_elasticsearch(current_user_id: str = Depends(get_current_user_id)):
