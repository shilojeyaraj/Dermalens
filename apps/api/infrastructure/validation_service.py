...
class SkincareValidationService:
    ...
    def __init__(self):
        self.skin_conditions = [
            "acne", "blackheads", "whiteheads", "dark_spots", "hyperpigmentation",
            "wrinkles", "dry_skin", "oily_skin", "sensitive_skin", "normal_skin",
            "rosacea", "eczema", "large_pores", "uneven_texture", "dull_skin"
        ]
        self.skin_types = ["dry", "oily", "combination", "normal", "sensitive"]
        self.product_types = ["cleanser", "serum", "moisturizer", "sunscreen", "exfoliant", "toner", "mask"]
        
        # Safety guidelines
        self.safety_guidelines = {
            "acids": {
                "max_daily": 1,
                "concentration_limits": {"salicylic_acid": 2.0, "glycolic_acid": 10.0, "lactic_acid": 10.0},
    ...
    def validate_skincare_routine(self, routine: Dict[str, Any], user_profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            morning_validation = self._validate_routine_steps(morning_routine, "morning", user_profile)
            evening_validation = self._validate_routine_steps(evening_routine, "evening", user_profile)
    ...
    def _validate_routine_steps(self, steps: List[Dict], routine_type: str, user_profile: Dict) -> Dict[str, Any]:
        ...
        for i, step in enumerate(current_order):
            if i < len(expected_order):
                ...
                if expected not in step and i > 0:
                    ...
                    validation["recommendations"].append(f"Consider following the order: {', '.join(expected_order)}")
        
        return validation
    
    def _validate_ingredient_conflicts(self, steps: List[Dict], user_profile: Dict) -> Dict[str, Any]:
        ...
        for ingredient in all_ingredients:
            ...
            if any(acid in ingredient_lower for acid in ["salicylic", "glycolic", "lactic"]):
                if any(conflict in " ".join(all_ingredients).lower() for conflict in ["retinol", "vitamin c"]):
                    validation["warnings"].append(f"{ingredient} may conflict with other active ingredients")
                    validation["recommendations"].append("Use acids and other actives on alternate nights")
            
            # Check retinol conflicts
            if "retinol" in ingredient_lower:
                if any(conflict in " ".join(all_ingredients).lower() for conflict in ["vitamin c", "acid"]):
                    validation["warnings"].append("Retinol may conflict with vitamin C or acids")
                    validation["recommendations"].append("Use retinol on alternate nights from other actives")
            
            # Check vitamin C conflicts
            if "vitamin c" in ingredient_lower or "ascorbic" in ingredient_lower:
                if any(conflict in " ".join(all_ingredients).lower() for conflict in ["retinol", "niacinamide"]):
                    validation["warnings"].append("Vitamin C may conflict with retinol or niacinamide")
                    validation["recommendations"].append("Use vitamin C in the morning, other actives at night")
        
        return validation
    
    def _validate_skin_type_compatibility(self, steps: List[Dict], user_profile: Dict) -> Dict[str, Any]:
        ...
        for step in steps:
            ...
            if skin_type == "sensitive":
                if any(ingredient in product for ingredient in ["acid", "retinol", "vitamin c"]):
                    ...
                    validation["recommendations"].append("Consider gentler alternatives for sensitive skin")
            
            elif skin_type == "oily":
                if "oil" in product and "cleanser" not in action:
                    validation["warnings"].append(f"{product} may be too heavy for oily skin")
                    validation["recommendations"].append("Consider oil-free or lightweight alternatives")
    ...
    def _validate_condition_targeting(self, steps: List[Dict], user_profile: Dict) -> Dict[str, Any]:
    ...
    def _extract_ingredients_from_product(self, product_name: str) -> List[str]:
        ...
        ingredient_keywords = [
            "salicylic", "glycolic", "lactic", "vitamin c", "retinol", "niacinamide",
            "hyaluronic", "glycerin", "ceramides", "peptides", "aloe", "centella"
        ]
    ...
    def _calculate_effectiveness_score(self, routine: Dict, user_profile: Dict) -> int:
        ...
        if any("sunscreen" in step.get("action", "").lower() for step in morning_steps):
            score += 20
        if any("moisturizer" in step.get("action", "").lower() for step in morning_steps + evening_steps):
            score += 15
        ...
        for condition in conditions:
            condition_lower = condition.lower()
            if condition_lower in condition_ingredients:
                if any(ingredient in all_products for ingredient in condition_ingredients[condition_lower]):
    ...
    def validate_product_recommendation(self, product: Dict[str, Any], user_profile: Dict) -> Dict[str, Any]:
        ...
        if skin_type == "sensitive":
            if any(ingredient in product_ingredients for ingredient in ["alcohol", "fragrance", "parabens"]):
            ...
            if any(ingredient in product_ingredients for ingredient in ["clay", "alcohol", "astringent"]):
