...
from config import GOOGLE_CLOUD_PROJECT, VERTEX_AI_CACHE_ENABLED
...
class IntelligentCachingService:
        ...
        try:
            ...
            if self.redis_client:
                await self.redis_client.setex(
                    cache_key, cache_data["ttl"], json.dumps(cache_data, default=str)
                )
    ...
    async def get_recommendation_cache(
        self, conditions: List[str], user_profile: Optional[Dict[str, Any]] = None
    ) -> Optional[Dict[str, Any]]:
            ...
            if similar_keys:
                ...
                if cached_data:
                    ...
                    if similarity_score > 0.7:  # Only use if very similar
                        logger.info(
                            f"🔍 Found similar analysis with {similarity_score:.2f} similarity"
                        )
    ...
    def _generate_recommendation_cache_key(
        ...
        content = json.dumps(
            {"conditions": sorted(conditions), "user_profile": user_profile or {}}, sort_keys=True
        ).encode()
