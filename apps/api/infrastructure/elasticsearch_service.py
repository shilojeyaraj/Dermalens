...
class ElasticsearchProductService:
    ...
    def _ensure_connected(self):
        ...
        try:
            ...
            if self.elasticsearch_api_key:
                ...
                self.es = Elasticsearch([self.elasticsearch_url], request_timeout=5, max_retries=0)
                ...
                mapping = {
                    "mappings": {
                        "properties": {
                            ...
                            "allergen_free": {"type": "boolean"},
                            "fragrance_free": {"type": "boolean"},
                            "cruelty_free": {"type": "boolean"},
                            "vegan": {"type": "boolean"},
                            "spf_level": {"type": "integer"},
    ...
    def index_product(self, product: Dict[str, Any]) -> str:
        ...
        try:
            ...
            product_id = product.get(
                "id", f"{product['brand']}_{product['name']}".lower().replace(" ", "_")
            )

            response = self.es.index(index=self.index_name, id=product_id, body=product)
        ...
        try:
            ...
            if skin_conditions:
                search_body["query"]["bool"]["filter"].append(
                    {"terms": {"skin_conditions": skin_conditions}}
                )

            # Filter by skin types
            if skin_types:
                search_body["query"]["bool"]["filter"].append({"terms": {"skin_types": skin_types}})
    ...
    def get_recommendations(
        self, user_profile: Dict[str, Any], analysis_results: List[Dict[str, Any]], limit: int = 10
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if detected_conditions:
                search_body["query"]["bool"]["filter"].append(
                    {"terms": {"skin_conditions": detected_conditions}}
                )

            # Filter by skin type
            if skin_type:
                search_body["query"]["bool"]["filter"].append(
                    {"term": {"skin_types": skin_type.lower()}}
                )
    ...
    def _get_recommendation_reason(
        self, product: Dict[str, Any], detected_conditions: List[str], user_profile: Dict[str, Any]
    ) -> str:
    ...
    def get_similar_products(self, product_id: str, limit: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "similar_products": similar_products}

        except Exception as e:
            logger.error(f"Error getting similar products: {e}")
            return {"success": False, "error": str(e), "similar_products": []}
    ...
    def get_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            agg_response = self.es.search(index=self.index_name, body=agg_body)
